...
export function OwnerLine({ parcel }: { parcel: ParcelRecord | undefined }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
      <p className="min-w-0 truncate text-[13.5px] font-semibold text-white/90">
        {parcel.ownerName}
      </p>
      {entity && (
        <span className="shrink-0 rounded-full bg-white/10 px-2 py-0.5 text-[10.5px] uppercase tracking-wider text-white/60">
          {entity}
        </span>
      )}
