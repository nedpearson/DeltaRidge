  ...
  if (points.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-white/10 px-5 py-10 text-center">
        <p className="font-display text-sm tracking-wide text-white/70">No trail for this route</p>
  ...
  return (
    <div>
      ...
      >
            ...
            {(Object.keys(MAP_STYLES) as MapStyleKey[]).map((key) => (
              <button
                ...
                className={`rounded px-2 py-1 text-[11px] font-medium ${
                  style === key ? 'bg-white/20 text-white' : 'text-white/60'
                }`}
              >
        ...
        {!image && (
          <div className="absolute bottom-2 left-2 rounded bg-black/55 px-2 py-1 text-[10px] text-white/50">
