...
const TONE: Record<CheckState, { dot: string; text: string; word: string }> = {
  established: { dot: 'bg-emerald-400', text: 'text-slate-600', word: 'Established' },
  reported: { dot: 'bg-slate-200', text: 'text-slate-600', word: 'Reported' },
  stale: { dot: 'bg-amber-400', text: 'text-slate-600', word: 'Ageing' },
  absent: { dot: 'bg-slate-200', text: 'text-slate-600', word: 'None' },
  attention: { dot: 'bg-red-400', text: 'text-slate-600', word: 'Needs attention' },
}
...
export default function IntegrityPanel({ evidence }: { evidence: IntegrityEvidence }) {
  ...
  return (
    <Card>
      ...
      <p
        className={`mt-2 text-sm ${summary.needsAttention > 0 ? 'text-red-300' : 'text-slate-600'}`}
      >
        {summary.sentence}
      </p>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                  ...
                  {/* The state is spelled out rather than left to the colour, so it
                      ...
                      amber from green. */}
                  <span className="text-xs text-slate-600">{tone.word}</span>
                </div>
                <p className="text-xs text-slate-600">{check.basis}</p>
              </div>
            </li>
          )
        })}
