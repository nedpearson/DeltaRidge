...
const TONE: Record<CheckState, { dot: string; text: string; word: string }> = {
  established: { dot: 'bg-status-success', text: 'text-text-secondary', word: 'Established' },
  reported: { dot: 'bg-bg-elevated', text: 'text-text-secondary', word: 'Reported' },
  stale: { dot: 'bg-status-warning', text: 'text-text-secondary', word: 'Ageing' },
  absent: { dot: 'bg-bg-elevated', text: 'text-text-secondary', word: 'None' },
  attention: { dot: 'bg-status-critical', text: 'text-text-secondary', word: 'Needs attention' },
}
...
export default function IntegrityPanel({ evidence }: { evidence: IntegrityEvidence }) {
  ...
  return (
    <Card>
      ...
      <p
        className={`mt-2 text-sm ${summary.needsAttention > 0 ? 'text-status-critical' : 'text-text-secondary'}`}
      >
        {summary.sentence}
      </p>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                  ...
                  {/* The state is spelled out rather than left to the colour, so it
                      ...
                      amber from green. */}
                  <span className="text-xs text-text-secondary">{tone.word}</span>
                </div>
                <p className="text-xs text-text-secondary">{check.basis}</p>
              </div>
            </li>
          )
        })}
