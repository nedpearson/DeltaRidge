...
export default function RoutesTab({
  ...
  if (routes.length === 0) {
    return loading ? (
      <Card>
        <p className="text-[12.5px] text-text-secondary">Reading routes…</p>
      </Card>
    ) : (
      <Nothing
...
function OneRoute({
  ...
  return (
    <div className="space-y-3">
      <button onClick={onBack} className="text-[12.5px] font-medium text-text-secondary">
        ← All routes
      </button>
      ...
      <Card>
        ...
        <div className="mt-3 space-y-1 border-t border-border-subtle pt-2 text-[11.5px] leading-relaxed text-text-secondary">
          <p>
          ...
          <div className="mt-2 space-y-2.5">
            {signals.map((signal) => (
              <div key={signal.code}>
                ...
                <p className="mt-0.5 text-[11.5px] leading-relaxed text-text-secondary">{signal.check}</p>
              </div>
            ))}
          </div>
          <p className="mt-2.5 border-t border-border-subtle pt-2 text-[11px] leading-relaxed text-text-secondary">
      ...
      {loading ? (
        <Card>
          <p className="text-[12.5px] text-text-secondary">Reading the trail…</p>
        </Card>
      ) : (
