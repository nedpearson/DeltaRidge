...
export default function SettingsTab({
  ...
  return (
    <div className="space-y-3">
      ...
      <Card>
        <p className="text-[12px] font-semibold text-text-secondary">MODE</p>
          ...
          {MODES.map((mode) => (
            <button
              ...
              className={`w-full rounded-xl px-3 py-2.5 text-left ring-1 ${
                draft.mode === mode.id ? 'bg-gold-500/15 ring-gold-400/40' : 'bg-bg-page hover:bg-bg-elevated ring-border-subtle'
              }`}
            >
              <p className="text-[13px] font-semibold">{mode.label}</p>
              <p className="mt-0.5 text-[11.5px] leading-relaxed text-text-secondary">{mode.detail}</p>
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card>
        <p className="text-[12px] font-semibold text-text-secondary">WEIGHTS</p>
        <div className="mt-2 space-y-2">
        ...
        <p
          className={`mt-2 text-[11.5px] ${balanced ? 'text-text-secondary' : 'text-status-warning/80'}`}
        >
          {balanced
            ? 'Adds to 100%.'
      ...
      <Card>
        <p className="text-[12px] font-semibold text-text-secondary">THRESHOLDS</p>
        <div className="mt-2 space-y-2">
          <Field
      ...
      <Card>
        <p className="text-[12px] font-semibold text-text-secondary">WHAT IS NEVER GRADED ON</p>
        ...
        <ul className="mt-2 space-y-0.5">
          {NEVER_GRADED_ON.map((item) => (
            <li key={item} className="text-[11.5px] text-text-secondary">
              · {item}
            </li>
          ))}
        </ul>
